...
public class TestLinkAPIHelper implements TestLinkAPIConst
{
			...
			if ( id != null ) {
				...
				for ( int c = 0; c < cases.size(); c++ ) {
					...
					if ( currentTestCase.compareTo(testCaseID) == 0 ) {
						Map caseinfo = (Map) cases.getData(c);
						return caseinfo; //(Map) cases.getData(c);
					}
				}
			}
		}
	 ...
	 * @throws TestLinkAPIException
	 */
	public static String getTestCaseExternalID(
		TestLinkAPIClient apiClient, 
		Integer projectID, 
		Integer testCaseID) throws TestLinkAPIException
	{
		...
		for ( int i = 0; i < suites.size(); i++ ) {
			...
			if ( id != null ) {
				...
				for ( int c = 0; c < cases.size(); c++ ) {
					...
					if ( currentTestCase.compareTo(testCaseID) == 0 ) {
						Map caseinfo = (Map) cases.getData(c);
						Object extId = caseinfo.get("external_id");
//						System.out.println(""+extId.toString());
						return extId.toString();
					}
				}
			}
		}
