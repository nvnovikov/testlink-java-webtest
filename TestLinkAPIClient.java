...
public class TestLinkAPIClient implements TestLinkAPIConst
{
	...
	Map cacheList = new HashMap<String, Map>();
...
//		List<HashMap<String, Object>> steps,
	...
	/**
	 * Upload a new test suite attachment using all the variables that are provided by
	 * the TestLink API. For more information on the parameters refer to the 
	 * TestLink API documentation.
	 * 
	 * @param devKey					Required	//Developer key 
	 * @param testsuiteid				Required	//Test Suite ID
	 * @param fileName					Required	//The file name of the Attachment (e.g.:notes.txt)
	 * @param fileType					Required	//The file type of the Attachment (e.g.: text/plain)
	 * @param content					Required	//The content (Base64 encoded) of the Attachment
	 * @param title						Optional	//(Optional) The title of the Attachment
	 * @param description				Optional	//(Optional) The description of the Attachment
	 * @author nnovikov
	 * 
	 * @since 1.9beta6
	 * @return PHP: mixed $resultInfo an array containing the fk_id, fk_table, title, 
	...
	public Integer uploadTestSuiteAttachment(
		String devKey,
		Integer testsuiteID,
		String fileName,
		String fileType,
...
//		setParam(params, REQUIRED, API_PARAM_TEST_SUITE_ID, suiteID);
		setParam(params, REQUIRED, API_PARAM_FILE_NAME, fileName);
		setParam(params, REQUIRED, API_PARAM_FILE_TYPE, fileType);
		setParam(params, REQUIRED, API_PARAM_CONTENT, content);
		setParam(params, OPTIONAL, API_PARAM_TITLE, title);
		setParam(params, OPTIONAL, API_PARAM_DESCRIPTION, description);
		executeUploadXmlRpcMethod(API_METHOD_UPLOAD_ATTACHMENT_SUITE, params); //executeUploadXmlRpcMethod
		// The id returned in results is the id within
		// the project we want the actual test case id
		return null; //TestLinkAPIHelper.getCaseIDByName(this, projectID, suiteID, caseName);
	}
	
	/**
	 * Upload a new test case attachment using all the variables that are provided by
	 * the TestLink API. For more information on the parameters refer to the 
	 * TestLink API documentation.
	 * 
	 * @param devKey					Required	//Developer key 
	 * @param testcaseid				Required	//Test Case INTERNAL ID
	 * @param fileName					Required	//The file name of the Attachment (e.g.:notes.txt)
	 * @param fileType					Required	//The file type of the Attachment (e.g.: text/plain)
	 * @param content					Required	//The content (Base64 encoded) of the Attachment
	 * @param title						Optional	//(Optional) The title of the Attachment
	...
	public Integer uploadTestCaseAttachment(
		String devKey,
		Integer testcaseID,
		String fileName,
		String fileType,
...
//		setParam(params, REQUIRED, API_PARAM_TEST_SUITE_ID, suiteID);
		...
		setParam(params, OPTIONAL, API_PARAM_TITLE, title);
		setParam(params, OPTIONAL, API_PARAM_DESCRIPTION, description);
		executeUploadXmlRpcMethod(API_METHOD_UPLOAD_ATTACHMENT_CASE, params); //executeUploadXmlRpcMethod
		// The id returned in results is the id within
		// the project we want the actual test case id
		return null; //TestLinkAPIHelper.getCaseIDByName(this, projectID, suiteID, caseName);
	}
	...
	/*
	 * Private UPLOAD method used to make xml-rpc method calls
	 * to the TestLink api URL.
	 * @author nnovikov
	 * The method is inherited from executeXmlRpcMethod
	 * and modified to eliminate doubling of uploading files!
	 */
	private TestLinkAPIResults executeUploadXmlRpcMethod(String method,
			Hashtable executionData) throws TestLinkAPIException {
		TestLinkAPIResults results = new TestLinkAPIResults();
		ArrayList params = new ArrayList();
		XmlRpcClient rpcClient = getRpcClient();
		...
		// Try without casting to an Object[] list since XML-RPC officially
		// returns
		// an Object but TestLink API is known to return Object[] list and cast
		// is
		// tried first and then if that does not work we go with Object.
		try {
			Object single = rpcClient.execute(method, params);
			...
			while (paramNames.hasNext()) {
				Object paramName = paramNames.next();
				msg = msg + "\n    " + paramName + "='"
						+ executionData.get(paramName) + "'";
			}
			ee.printStackTrace();
			throw new TestLinkAPIException(msg, ee);
		}
		return results;
	}
